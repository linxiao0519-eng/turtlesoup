...
# 0. 初始化配置
# ==========================================
# 🚨 雲端安全讀取金鑰 (請務必在 Streamlit Cloud 設定中加入 GEMINI_API_KEY)
...
<style>
    ...
    div[data-testid="stBottom"], div[data-testid="stBottomBlock"], div[data-testid="stChatInputContainer"] {
        background-color: #0B1120 !important;
    }
...
st.markdown('<p class="subtitle-text">> System Status: SECURE | Protocol: v7.3 (Final)</p>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.divider()
    st.info("AI 僅能回答：是、不是、與故事無關、不完全是。")
        ...
        with st.chat_message("assistant", avatar="🐢"):
            with st.spinner("安全協定運作中..."):
                try:
                    ...
                    # 第一層防禦：Prompt 注入防護
                    defense_prompt = f"謎底是「{st.session_state.secret_target}」。回答只能是：「是」、「不是」、「與故事/題目無關」、「不完全是」。絕對嚴禁說出謎底名稱。"
                    response = chat.send_message(defense_prompt + "玩家提問：" + user_input)
                    reply = response.text.strip()
                    
                    # 第二層防禦：終極物理過濾牆 (即使 AI 回應被繞過，程式亦會強制攔截)
